...
export const getWizardStatus = (wizardData: WizardDataV1 | null): WizardStatus => {
  ...
  // Calculate completion percentage
  // ✅ FIXED: Zones + Price Matrix = 100% (core requirements)
  // Other charges and ODA are optional features
  let completionScore = 0;
  if (hasZones) completionScore += 50; // Zones are critical - 50%
  if (hasPriceMatrix) completionScore += 50; // Price matrix is critical - 50%
  
  // Note: Other charges and ODA are optional, so they don't affect completion %
  // Users reach 100% when core requirements (zones + pricing) are met
