...
// ✅ NEW: Validate primary contact fields (optional fields - only validate if provided)
    const primaryContactEmail = safeGetField(b, 'primaryContactEmail');
    if (primaryContactEmail && primaryContactEmail.length > 0) {
      let emailOk = false;
      try {
        emailOk = !!(isEmail.validate ? isEmail.validate(primaryContactEmail) : isEmail(primaryContactEmail));
      } catch {
        emailOk = EMAIL_FALLBACK_RE.test(primaryContactEmail);
      }
      if (!emailOk) {
        errs.push('Primary contact email format is invalid.');
      }
    }

    const primaryContactPhone = sanitizeDigitsOnly(safeGetField(b, 'primaryContactPhone'));
    if (primaryContactPhone && primaryContactPhone.length > 0 && primaryContactPhone.length !== 10) {
      errs.push('Primary contact phone must be 10 digits if provided.');
    }

    // ✅ NEW: Validate legal company name length if provided
    const legalCompanyName = safeGetField(b, 'legalCompanyName');
    if (legalCompanyName && legalCompanyName.length > 100) {
      errs.push('Legal company name must be at most 100 characters.');
    }
  ...
  const buildPayloadForApi = () => {
    ...
    // ✅ NEW: Extract primary contact fields
  const primaryContactName = capitalizeWords(
    safeGetField(basics, 'primaryContactName', 'contactName')
  ).slice(0, 100);
  
  const primaryContactEmail = safeGetField(
    basics, 
    'primaryContactEmail', 
    'contactEmail'
  ).trim().toLowerCase();
  
  const primaryContactPhone = sanitizeDigitsOnly(
    safeGetField(basics, 'primaryContactPhone', 'contactPhone')
  ).slice(0, 10);

  // ✅ NEW: Extract legal company name (fallback to regular name)
  const legalCompanyName = capitalizeWords(
    safeGetField(basics, 'legalCompanyName', 'legalName') || name
  ).slice(0, 100);
...
const payloadForApi = {
  ...
  companyName: name,
  legalCompanyName: legalCompanyName, // ✅ NEW
  vendorCode: vendorCode,
  vendorPhone: vendorPhoneNum,
  vendorEmail: vendorEmail,
  primaryContactName: primaryContactName, // ✅ NEW
  primaryContactEmail: primaryContactEmail, // ✅ NEW
  primaryContactPhone: primaryContactPhone, // ✅ NEW
  gstNo,
  mode: transportMode || 'road',
  serviceModes: serviceModes, // ✅ ALREADY EXISTS
  address,
  state: String(geo.state ?? '').toUpperCase(),
  city: String(geo.city ?? ''), // ✅ NEW
  pincode: pincodeNum,
  rating: companyRating,
  companyRating: companyRating, // ✅ ALREADY EXISTS
  subVendor: subVendor, // ✅ NEW
  volumetricUnit: volUnit, // ✅ NEW
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fd = new FormData();
      ...
      fd.append('legalCompanyName', payloadForApi.legalCompanyName || payloadForApi.companyName); // ✅ NEW
      ...
      fd.append('primaryContactName', payloadForApi.primaryContactName || ''); // ✅ NEW
      fd.append('primaryContactEmail', payloadForApi.primaryContactEmail || ''); // ✅ NEW
      fd.append('primaryContactPhone', payloadForApi.primaryContactPhone || ''); // ✅ NEW
      fd.append('gstNo', payloadForApi.gstNo);
      fd.append('mode', payloadForApi.mode);
      fd.append('serviceModes', payloadForApi.serviceModes || payloadForApi.mode); // ✅ NEW
      ...
      fd.append('rating', String(payloadForApi.rating || 3)); // ✅ CHANGED
      fd.append('subVendor', payloadForApi.subVendor || ''); // ✅ NEW
      fd.append('volumetricUnit', payloadForApi.volumetricUnit || 'cm'); // ✅ NEW
